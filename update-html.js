...
files.forEach(file => {
    ...
    // 2. Add elevation.css link
    if (!content.includes('elevation.css')) {
        content = content.replace('<link rel="stylesheet" href="animations.css">', '<link rel="stylesheet" href="animations.css">\\n    <link rel="stylesheet" href="elevation.css">');
    }

    // 3. Update nav code
    const navRegex = /<!--\s*Navigation\s*-->[\\s\\S]*?<\/nav>[\\s\\S]*?<div class="mobile-nav-overlay">[\\s\\S]*?<\/div>/;
    if (navRegex.test(content)) {
        let replacementNav = defaultNav;
        // Make the active link active based on file name
        ['index.html', 'collection.html', 'about.html', 'contact.html'].forEach(link => {
            if (file === link) {
                replacementNav = replacementNav.replace(\`<a href="\${link}">\`, \`<a href="\${link}" class="active">\`);
            }
        });
    ...
    content = content.replace(/<img\\s*src="logo\\.png"[\\s\\S]*?class="footer-logo-img"[^>]*>/g, '<img src="logo.png" alt="Choco Box Maroc" class="footer-logo-img">');
